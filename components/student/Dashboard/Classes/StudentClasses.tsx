...
const StudentClasses = ({
    ...
    return (
        <>
            <div
                className={`w-[290px] my-8 rounded-lg border-2 ${
                    classStatus === 'cancelled'
                        ? 'bg-[#FFF8F8] border-[#FEE5E3]'
                        : 'bg-[#F5F8F8] border-[#DBE6E3]'
                }`}
            >
                <div className='px-3 py-3.5 flex gap-3 relative h-[75px]'>
                    <div>
                        <div className='flex items-center gap-3'>
                            <div className='font-author text-[22px] font-extrabold'>
                                {classCode}
                            </div>
                            <div
                        ...
                        {classDate && (
                            <div className='flex font-author -mt-1'>
                                <div className='font-[400] border-r-2 border-[#D9D9D9] pr-[10px] text-[15px] 2xl:text-[17px]'>
                                    {moment(classDate, 'DD/MM/YYYY').format(
                                        'Do MMM, YYYY'
                                    )}
                                </div>
                                {scheduledAt && (
                                    <div className='font-[400] pl-[10px] text-[15px] 2xl:text-[17px]'>
                                        {scheduledAt}
                                    </div>
                                )}
                            </div>
                ...
                <div className='bg-white px-3 py-3.5'>
                    ...
                    <div className='font-author 2xl:text-[18px] line-clamp-3'>
                        {description}
                    </div>
                </div>
                <div
                ...
                >
                    <button className='bg-[#717688]  font-semibold px-[20px] py-[10px] rounded-[8px] text-white relative group overflow-hidden'>
                        <span className='relative z-40'>
                            <Link href={`/student/classes/${classId}?t=c`}>
                                VIEW CLASS DETAILS
                            </Link>
                        </span>

...
const SubsessionOptions = (props: SubsessionOptionsProps) => {
    ...
    const [isEnableCountdown, setIsEnableCountdown] = useState(
        waitSeconds !== 0
    )
            ...
            <Menu selectable={false} expandIcon={null}>
                <MenuItem key='1'>
                    <Checkbox
                        ...
                        onChange={(event) =>
                            setIsBackToMainSessionEnabled(event.target.checked)
                        }
                    >
                        Allow participants to return the main session at any
                        time
                    </Checkbox>
                </MenuItem>
                <MenuItem key='2'>
                    <Checkbox
                        checked={isAutoJoinSubsession}
                        onChange={(event) =>
                            setIsAutoJoinSubsession(event.target.checked)
                        }
                    >
                        Automatically move all assigned participants into
                        subsessions
                    </Checkbox>
                </MenuItem>
                <Menu.Divider />
                ...
                <MenuItem key='4' className='indent'>
                    <Checkbox
                        ...
                        disabled={!isTimerEnabled}
                        onChange={(event) =>
                            setIsTimerAutoEnabled(!event.target.checked)
                        }
                    >

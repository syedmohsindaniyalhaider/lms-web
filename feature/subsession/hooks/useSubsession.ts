...
export function useSubsession(
    ...
    const [subsessionStatus, setSubsessionStatus] = useState<SubsessionStatus>(
        SubsessionStatus.NotStarted
    )
    const [userStatus, setUserStatus] = useState<SubsessionUserStatus>(
        SubsessionUserStatus.Initial
    )
    const [currentSubsession, setCurrentSubsession] =
        useState<CurrentSubsession>({
            subsessionName: '',
            subsessionId: '',
    ...
    const onParticipantsChange = useCallback(
        (
            _participants: any,
            currentUpdates?: ParticipantPropertiesPayload[]
        ) => {
            if (ssClient) {
    ...
    useEffect(() => {
        if (
            ssClient &&
            previousSubsessions &&
            previousSubsessions !== subsessions
        ) {
            const assignedUserList = subsessions.reduce(
                (prev: any[], subsession: Subsession) => {
            ...
            if (
                ...
                unassignedUserList = unassignedUserList.filter(
                    (u: any) => !assignedUserList.includes(u.userId)
                )
            }
    ...
    const createSubsessions = useCallback(
        async (
            ...
            if (ssClient) {
                try {
                    const subsessions = await ssClient.createSubsessions(
                        subsessionNumber,
                        pattern
                    )
    ...
    const addSubsession = useCallback(async () => {
        if (ssClient) {
            try {
                const newSubsessions = await ssClient.createSubsessions(
                    1,
        ...
        (
            ...
            setSubsessions(
                produce((subsessions: Subsession[]) => {
                    const subsession = subsessions.find(
                        (r: Subsession) => r.subsessionId === subsessionId
                    )
                    if (subsession) {
            ...
            setSubsessions(
                produce((rooms: Subsession[]) => {
                    const room = rooms.find(
                        (r: Subsession) => r.subsessionId === targetSubsessionId
                    )
                    const sourceRoom = rooms.find(
                        (r: Subsession) => r.subsessionId === sourceSubsessionId
                    )
                    const index = sourceRoom?.userList.findIndex(
                        (u) => u.userId === userId
                    )
                    if (index !== undefined) {

...
import { IconFont } from '../../components/ui/Zoom/IconFont'
...
const SubsessionContainer = (props: any) => {
    ...
    const {
        ...
        setInviteVisible,
        setInviteAccepted,
    } = useInviteJoinSubsession(zmClient)
    const { formattedSubsessionCountdown } = useSubsessionCountdown(
        zmClient,
        subsessionClient
    )
    ...
    useSubsessionTimeUp(
        ...
        subsessionClient,
        isHost,
        subsessionOptions.timerDuration
    )
    useAskForHelp(zmClient, subsessionClient)
    const closingCountdown = useSubsessionClosingCountdown(
        zmClient,
        subsessionStatus
    )
    ...
    return (
        <div className='breakout-room-viewport'>
            ...
            {userStatus === SubsessionUserStatus.InSubsession && (
                <h2 className='room-info'>
                    You are in {currentSubsession.subsessionName}.
                </h2>
            )}
            {(isHost ||
                ...
                <Button
                    ...
                    onClick={() => {
                        if (isHost) {
                            ...
                            subsessionClient?.joinSubsession(
                                currentSubsession.subsessionId
                            )
                        } else {
                            if (invitedToJoin?.accepted) {
                                subsessionClient?.joinSubsession(
                                    invitedToJoin.subsessionId
                                )
                            } else {
            ...
            {isHost && (
                ...
                >
                    ...
                    subsessionStatus === SubsessionStatus.NotStarted ? (
                        <SubsessionCreate
                            totalParticipantsSize={participantsSize}
                            onCreateSubsession={createSubsessions}
                        />
                    ) : (
                        <SubsessionManage
            ...
            {!isHost && subsessionRemainingTitle && (
                <div className='room-remaining'>{subsessionRemainingTitle}</div>
            )}
            {!isHost && closingCountdown >= 0 && (
                ...
                >
                    <p style={{ fontWeight: 700 }}>
                        {' '}
                        Subsessions will close in {closingCountdown} seconds
                    </p>
                    <p>

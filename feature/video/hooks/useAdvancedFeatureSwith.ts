...
export const useAdvancedFeatureSwitch = (
    ...
    const [advancedSwitch, setAdvancedSwitch] = useState<
        Record<string, AdvancedFeatureSwitch>
    >({})
    useParticipantsChange(zmClient, (participants, updatedParticipants) => {
        if (!updatedParticipants) {
                ...
                Object.assign(payload, {
                    [`${current.userId}`]: {
                        ...
                        farEndCameraControl: {
                            ...
                            enabled:
                                current.bVideoOn &&
                                mediaStream?.getFarEndCameraPTZCapability(
                                    current.userId
                                )?.pan,
                        },
                    },
            ...
            const concernedItems = updatedParticipants.filter(
                (item) =>
                    item.audio !== undefined || item.bVideoOn !== undefined
            )
                ...
                if (Object.hasOwn(advancedSwitch, `${userId}`)) {
                    ...
                    setAdvancedSwitch(
                        produce((draft) => {
                            ...
                            if (item.audio !== undefined) {
                                initState.adjustVolumn.enabled =
                                    item.audio === 'computer'
                            } else if (item.bVideoOn !== undefined) {
        ...
        setAdvancedSwitch(
            produce((draft) => {
                ...
                if (userItem !== undefined) {
                    draft[`${userId}`].adjustVolumn.toggled =
                        !draft[`${userId}`].adjustVolumn.toggled
                }
            })
        )
    ...
    const toggleFarEndCameraControl = useCallback((userId: number) => {
        setAdvancedSwitch(
            produce((draft) => {
                const userItem = draft[`${userId}`]
                if (userItem !== undefined) {
                    draft[`${userId}`].farEndCameraControl.toggled =
                        !draft[`${userId}`].farEndCameraControl.toggled
                }
            })
        )

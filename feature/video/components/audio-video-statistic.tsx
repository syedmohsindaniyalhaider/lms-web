    ...
    return streamMertics.map((metrics, index) => {
        ...
        if (encodingData) {
            ...
            if (Array.isArray(metrics.value)) {
                value = metrics.value.map(
                    (m: string) => (encodingData as { [key: string]: any })[m]
                )
            } else {
        ...
        if (decodingData) {
            ...
            if (Array.isArray(metrics.value)) {
                value = metrics.value.map(
                    (m: string) => (decodingData as { [key: string]: any })[m]
                )
            } else {
...
const AudioVideoStatisticModel = (props: AudioVideoStatisticModelProps) => {
    ...
    const [audioEncodingStatistic, setAudioEncodingStatistic] =
        useState<AudioQosData>()
    const [audioDecodingStatistic, setAudioDecodingStatistic] =
        useState<AudioQosData>()
    const [videoEncodingStatistic, setVideoEncodingStatistic] =
        useState<VideoQosData>()
    const [videoDecodingStatistic, setVideoDecodingStatistic] =
        useState<VideoQosData>()
    ...
    const audioDataSource = getDataSouce(
        AudioMetrics,
        audioEncodingStatistic,
        audioDecodingStatistic
    )
    const videoDataSource = getDataSouce(
        VideoMetrics,
    ...
    useMount(() => {
        if (mediaStream) {
            const { encode: audioEncoding, decode: audioDecoding } =
                mediaStream.getAudioStatisticData()
            const { encode: videoEncoding, decode: videoDecoding } =
                mediaStream.getVideoStatisticData()
